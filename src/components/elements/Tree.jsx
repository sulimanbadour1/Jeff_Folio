import styles from "../../stylesheets/elements/Tree.module.css";
...
const Tree = () => {
  ...
  useEffect(() => {
    ...
    const createHeading = (level, content) => {
      const Tag = `h${Math.min(level, 6)}`; // Ensure level is within heading tag limits (h1-h6)
      return <Tag>{content}</Tag>;
    };

    const getTopicTemplate = ({ level, prevRotation }) => {
      return ({ title, items }, i = 0, a = [""]) => {
        ...
        const fixedRotation =
          totalRotation < -90 || totalRotation > 90 ? -180 : 0;
        ...
        return (
          ...
          >
            ...
            <div className="items">{items.map(topicTemplate)}</div>
          </details>
        );
      };
    };
    ...
    setTopicsHtml(topicTemplate(topics));
  }, []);

  return (
    <div className="container">
      <div dangerouslySetInnerHTML={{ __html: topicsHtml }} />
    </div>
  );
};
